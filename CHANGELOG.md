# Changelog

## 1.1.7

- Removed the shared native top fade that rendered as a white horizontal separator on Settings, new chats, and Scheduled tasks.
- Kept the main surfaces continuous with the Sayram Lake background while preserving the readable settings shell and input surfaces.
- Repacked the portable theme package.

## 1.1.6

- Restyled the Scheduled tasks sticky search band so it no longer renders as an opaque white strip over the landscape.
- Removed the gold focus rectangle from the Scheduled tasks search field while keeping the native input shape.
- Removed the Scheduled tasks top fade and search-band pseudo-element so the page no longer shows unexplained white horizontal bars.
- Recolored the Scheduled tasks Create action for readable contrast.
- Hid inactive model-picker endpoint and ultra-usage warning layers that were incorrectly forced visible and overlapped Reset to default.
- Repacked the portable theme package.

## 1.1.5

- Styled the settings route's outer surface with the theme palette instead of the native white shell.
- Added balanced padding and a single calm frame around the Profile Token activity chart.
- Removed the theme gold focus rectangle from the settings search field while preserving the native input shape.
- Raised contrast for settings navigation section labels and the Back to app control.
- Repacked the portable theme package.

## 1.1.4

- Removed the hidden runtime marker from normal document flow so opening a side file cannot create an extra page-height scroll offset.
- Repacked the portable theme package.
- Added standalone Windows launch, apply, and restore scripts for public distribution.

## 1.1.3

- Improved activity-row spacing and removed nested activity surfaces.
- Preserved readable old-chat content, link colors, diff colors, and composer focus behavior.
